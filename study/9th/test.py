...
def left(i,j,v):
    ...
    if 0<=i-1<R and 0<=j<C and 0<=i-1<R and 0<=j-1<C :
        if not (((i,j) in wall and (i-1,j) in wall) or((i-1,j) in wall and (i-1,j-1) in wall)) and visited[i-1][j-1] == 0 :
            left(i-1,j-1,v-1)
    # 왼
    if 0 <= i < R and 0 <= j - 1 < C :
        if not ((i,j) in wall and (i,j-1) in wall) and visited[i][j-1] == 0 :
            left(i,j-1,v-1)
    # 왼대각 아래
...
def up(i,j,v):
    ...
    if 0 <= i < R and 0 <= j - 1 < C and 0 <= i - 1 < R and 0 <= j - 1 < C:
        if not(((i,j) in wall and (i,j-1) in wall) or ((i,j-1) in wall and (i-1,j-1) in wall)) and visited[i-1][j-1] == 0 :
            up(i-1,j-1,v-1)
    # 위
    if 0 <= i - 1 < R and 0 <= j < C :
        if not ((i,j) in wall and (i-1,j) in wall) and visited[i-1][j] == 0 :
            up(i-1,j,v-1)
    # 오른 대각위
    if 0 <= i < R and 0 <= j + 1 < C and 0 <= i - 1 < R and 0 <= j + 1 < C:
        if not(((i,j) in wall and (i,j+1) in wall) or ((i,j+1) in wall and (i-1,j+1) in wall)) and visited[i-1][j+1] == 0 :
            ...
            if graph[i][j] != 0:
                for di,dj in point :
                    ...
                    if 0<=ni<R and 0<=nj<C and graph[i][j] > graph[ni][nj] :
                        if (i,j) in wall and (ni,nj) in wall and (i,j,ni,nj) in wall_v : continue
